...
public class ProjectCountBolt extends BaseWindowedBolt {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ProjectCountBolt.class);
	...
	public void execute(TupleWindow inputWindow) {
		...
		for (Tuple tuple : newTuples) {
			...
			// System.out.println("ProjectCountBolt: msgID " +
			// tuple.getValue(2));
		}
		...
		// // System.out.println("ProjectCountBolt : value " + entry.getValue()
		// + " key " + entry.getKey());
		// }

		ts = System.currentTimeMillis();
		collector.emit(new Values(counts, ts, "project_name"));
		System.out.println("ProjectCountBolt: Events in current window: "
				+ tuplesInWindow.size() + " timestamp: "
				+ System.currentTimeMillis() + " msgTS" + msgTS);
